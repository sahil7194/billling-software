...
<table class="table table-hover mt-1 mt-2">
  ...
  <tbody id="services_data_unit">
    ...
    $a=0;  
      $sql_for_entry_by_unit="SELECT dailystatusforunit.id, dailystatusforunit.`SeerviceId`, dailystatusforunit.`Quantity`, DATE_FORMAT(dailystatusforunit.`Date`,'%d %M %Y') as `Date`, dailystatusforunit.`time`, dailystatusforunit.doneby,dailystatusforunit.empty_quantity,office_deatils.office_name, office_deatils.code FROM dailystatusforunit INNER JOIN office_deatils ON dailystatusforunit.Office_id=office_deatils.id WHERE dailystatusforunit.`Date`='$date' ORDER BY office_deatils.code ASC";
