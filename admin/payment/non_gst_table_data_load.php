...
<table class="table table-hover">
  ...
  <tbody id="non_gst_table_data">
     <?php 
     $sr=0;   
        $sql_get_data="SELECT `id`,`invoice_number`, `company_id`, `invoice_data`, `total_amount`, `paid_amount`, `gst_status`,DATE(`create_at`) as`date` FROM `sells_invoice_data` WHERE `gst_status`='0' AND MONTH(`create_at`)='$date'";
           ...
           <tr>
              ...
              <td align="left">
                <a href="make_sells_payment.php?id=<?php echo $row_get_data['id']?>&company_id=<?php echo $row_get_data['company_id']?>&amount=<?php echo $row_get_data['total_amount']?>">
                     <button class="btn btn-primary" class="make_payment" id="<?php echo $row_get_data['id'];?>">
