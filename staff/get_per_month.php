...
?>
  ...
  <tbody id="services_data_month">
    ...
    $a=0;  
      $sql_for_entry_by_month="SELECT dailystatusformonth.id, dailystatusformonth.`SeerviceId`, dailystatusformonth.ServicesStatus, DATE_FORMAT(dailystatusformonth.Date,'%d %M %Y') as `Date`, dailystatusformonth.time,office_deatils.office_name, office_deatils.code FROM dailystatusformonth INNER JOIN office_deatils ON dailystatusformonth.Office_id=office_deatils.id WHERE dailystatusformonth.`Date`='$date' AND dailystatusformonth.doneby='$username' ORDER BY office_deatils.code ASC";
